...
Measured on GPT-2 (124M) with a T4 GPU (Google Colab), 200 WikiText-2 test paragraphs, 10 generation runs (3 warmup):
...
- **Memory: -32.5%** — the clearest win. INT8 quantization cut peak GPU memory from 728MB to 492MB.
- **Latency: +2.1%** (slightly slower, not faster). This is a real and worth-reporting finding: on a small model like GPT-2, the dequantization overhead at inference time outweighs any compute savings from lower precision. Quantization's speed benefit shows up more clearly on larger models and with hardware/kernels specifically optimized for INT8 matmul.
- **Quality: +0.49% perplexity** (essentially unchanged). Generated text (see notebook, section 8) stayed coherent and grammatical for all test prompts.

**Takeaway:** for this model size, INT8 quantization is a strictly memory-motivated optimization — worth it if you're deploying under a memory budget (e.g. fitting more model replicas per GPU, or running on constrained hardware), but it should not be expected to speed up generation on small models.
